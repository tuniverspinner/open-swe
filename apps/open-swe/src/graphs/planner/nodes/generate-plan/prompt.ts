...
6. **LangGraph-specific planning:**
   ...
   - **CRITICAL: Never create agent.py for existing projects** - If the codebase contains ANY existing LangGraph files (such as graph.py, main.py, app.py) or ANY files that import/export graphs, do NOT plan agent.py creation. Always work with the existing file structure.
   - Only create agent.py when building a completely new LangGraph project from an empty directory with zero existing graph-related files.
