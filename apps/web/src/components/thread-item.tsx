...
export function ThreadItem({ thread, onClick, variant = "dashboard", className }: ThreadItemProps) {
  ...
  return (
    <div
      className={cn(
        ...
        isSidebar ? "hover:bg-gray-50" : "rounded-lg p-4 shadow-sm transition-shadow hover:shadow-md",
        className
      )}
    ...
    >
      <div className={cn("flex items-center gap-2", !isSidebar && "gap-3")}>
        <div className={cn("flex-shrink-0", isSidebar ? "mt-0.5" : "mt-1")}>
          <StatusIndicator status={thread.status} size={isSidebar ? "sm" : "default"} />
        </div>
          ...
          {!isSidebar && (
            <div className="mt-2 flex items-center gap-4 text-xs text-gray-500">
              <Badge variant="outline" className="text-xs">
                {thread.completedTasksCount}/{thread.totalTasksCount} tasks completed
              </Badge>
            </div>
          )}
        </div>
